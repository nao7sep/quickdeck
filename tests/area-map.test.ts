...
// QuickDeck has, and which tests stand for each. A map nobody checks decays into a list of paths
// that used to exist, so this reads the real file and holds it to what is on disk. QuickDeck's paths
// are repository-relative, because its tests live in tests/ and in src-tauri/tests/.
...
describe("the area map", () => {
  ...
  it("names only tests that exist", () => {
    const missing = areas().flatMap((area) =>
      area.tests.filter((test) => !existsSync(path.join(ROOT, test))).map((test) => `${area.name}: ${test}`),
    );
